...
```{r}
#| code-fold: true
if (!dir.exists("output/Figure6")) dir.create("output/Figure6")
```
...
```{r}
umap_subset <- qs::qread("Figure6/umap_subset.qs")
summarized <- readRDS("Figure6/summarized.rds")
umap <- readRDS("Figure6/umap.rds")
clidata <- readRDS("Figure6/clidata.rds")
```

Helper function to create partial coordinates:
```{r}
...
```{r}
system.time({
    p1 <- ggheatmap(summarized$num, filling = FALSE) -
        # set default theme
        scheme_theme(
            plot.margin = margin(),
