# Overview of ggalign's visualization capabilities
...
data <- readxl::read_xlsx(
    "Figure3/Supplementary tables.xlsx",
    skip = 1L
)
...
## Figure B
Data was downloaded from https://academic.oup.com/mbe/article/38/9/4039/6294410

The data preparation code is collapsed in the source but can be expanded for
inspection.
```{r}
#| code-fold: true
#| output: false
alltax <- read.csv("Figure3/all_stain_taxonomy.csv")
...
linktab <- read.csv("Figure3/Interaction_link_tab.csv")
all(linktab$Inhibitor %in% trda$tip.label)
all(linktab$Sensitive %in% trda$tip.label)

weighttab <- read.csv("Figure3/Interaction_weight.csv")
tippoint <- read.csv("Figure3/stain_tippoint.csv")
...
BGCsda <- read.csv("Figure3/BGCs_heatmap.csv")
...
circle <- circle_discrete(
    ...
    # inner links ------------------------------------
    ggalign(trda$tip.label, size = 2) +
    scheme_data(function(data) {
        # for data, value is the rownames, we match it with linktab
        dplyr::mutate(
            linktab,
            x = data$.x[match(Inhibitor, data$value)],
            xend = data$.x[match(Sensitive, data$value)]
        )
    }) +
    ...
    ggalign(trda$tip.label, size = 0.1) +
    geom_point(aes(.x, 1, shape = Taxa, color = Taxa),
        data = function(data) {
            dplyr::inner_join(data, tippoint, by = c(value = "Isolation"))
        }
    ) +
    scale_shape_manual(
...
## Figure C
We utilize the classic `iris` dataset included in the R `datasets` package to
...
## Figure D
WE use the same data from `Figure B`.

To reduce complexity, we randomly sample 50 strains. The data preparation code
...
ggsave("output/Figure3/D.pdf", plot = upset, width = 8, height = 4)
```

## Figure E
This figure demonstrates a 3D heatmap visualization created from a small mock
expression dataset. We use the `biotidy::mockSE()` function to generate a 9x9 mock
matrix and apply log-transformed counts (log1p). 

```{r}
# Install 'biotidy' package from GitHub if not available
if (!requireNamespace("biotidy")) {
...
## Figure G
We compare transcriptomics and proteomics datasets simulated using the
`biotidy::mockSE()` function. The data preparation code is initially collapsed
in the source but can be expanded for inspection.
...
# Manually enforce partial correspondence between datasets
# to simulate realistic yet non-identical patterns
    ...
    cross_link(
        .link_draw(
            function(data) {
                ...
                # we have three groups, here, we created three color
                color <- RColorBrewer::brewer.pal(3L, "Dark2")
                grobs <- lapply(data, function(dd) {
                    # for each link, there are two data frame for each hand
                    # we map the color based on the left panel group (hand1)
                    grid::linesGrob(
                        c(dd$hand1$x, dd$hand2$x),
                        (c(dd$hand1$y, dd$hand2$y) +
                            c(dd$hand1$yend, dd$hand2$yend)) / 2L,
                        gp = gpar(col = color[dd$hand1$link_panel])
                    )
                })
                rlang::inject(grid::gList(!!!grobs))
            },
            # we link the first one to the first one
            !!!lapply(seq_len(9), function(i) rlang::new_formula(i, i))
        ),
        size = 0.5
    ) +
    theme(plot.margin = margin()) +
...
## Figure H
We compare two transcriptomics datasets simulated using the `biotidy::mockSE()`
function. The data preparation code is initially collapsed in the source but can
be expanded for inspection.
```{r}
#| code-fold: true
set.seed(1L)
