...
taxonomy <- data.table::fread("Figure4/taxonomy.txt")
phylum <- data.table::fread("Figure4/bacteria.sample.relabund.phylum.txt")
genus <- data.table::fread("Figure4/bacteria.sample.relabund.genus.txt")
metadata <- data.table::fread("Figure4/sample_metadata.txt")
...
```{r}
p4 <- stack_discreteh(t(phylum), sizes = c(0.1, 1, 1)) -
    # set default theme
    scheme_theme(
        plot.margin = margin(),
        panel.background = element_blank(),
...
phylum_test <- apply(combn(unique(metadata$project), 2L), 2L, function(pair) {
    ...
    ans <- apply(data, 2L, function(relabundance) {
        ans <- wilcox.test(relabundance ~ group,
            data = data.frame(
...
```{r}
p5 <- stack_crossh(t(phylum), sizes = c(0.1, 1, 1)) -
    # set default theme
    scheme_theme(
        plot.margin = margin(),
        panel.background = element_blank(),
